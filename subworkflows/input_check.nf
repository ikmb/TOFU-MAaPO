...
workflow input_check {
    ...
    if (hasExtension(params.reads, "csv")) {
        channel.from(file(params.reads))
            .splitCsv(header: true, sep: ',')
            .map { row ->

                def id = row.id ? row.id : false
                if (!id) {
                    exit(1, "Empty id in csv-input found")
                }
                def meta = [:]
                meta.id = id
                ...
                def readsize = [read1, read2, read3].count { read -> read }

                meta.single_end = readsize == 1 ? true : false

                if (!hasinternetprefix(read1)) {
                    // FILE IS LOCALLY AVAILABLE
                    if (!FileCheck.checkoutfile("${read1}")) {
                        exit(1, "Within the input csv the file ${read1} in column read1 does not exist for ${id}")
                    }
                    if (readsize >= 2) {
                        if (!FileCheck.checkoutfile("${read2}")) {
                            exit(1, "Within the input csv the file ${read2} in column read2 does not exist for ${id}")
                        }
                    }
                    if (readsize == 3) {
                        if (!FileCheck.checkoutfile("${read3}")) {
                            exit(1, "Within the input csv the file ${read3} in column read3 does not exist for ${id}")
                        }
                    }
                    if (!read1) {
                        exit(1, "Invalid input samplesheet in at least column read1! Is your csv file comma separated?")
                    }
                    if (!(hasExtension(read1, ".fastq.gz") || hasExtension(read1, ".fq.gz"))) {
                        exit(1, "Invalid file ${read1} ! Reads need to end with .fastq.gz")
                    }

                    if (readsize >= 2) {
                        if (!read2) {
                            exit(1, "Invalid input samplesheet in at least column read2")
                        }
                        if (!FileCheck.checkoutfile("${read2}")) {
                            exit(1, "Within the input csv the file ${read2} in column read2 does not exist for ${id}")
                        }
                        if (!(hasExtension(row.read2, ".fastq.gz") || hasExtension(row.read2, ".fq.gz"))) {
                            exit(1, "Invalid file ${read2} ! Reads need to end with .fastq.gz")
                        }
                    }
                    if (readsize == 3) {
                        if (!read3) {
                            exit(1, "Invalid input samplesheet in at least column read3")
                        }
                        if (!FileCheck.checkoutfile("${read3}")) {
                            exit(1, "Within the input csv the file ${read3} in column read3 does not exist for ${id}")
                        }
                        if (!(hasExtension(row.read3, ".fastq.gz") || hasExtension(row.read3, ".fq.gz"))) {
                            exit(1, "Invalid file ${read3} ! Reads need to end with .fastq.gz")
                        }
                    }
                }
                else {
                ...
                if (params.assemblymode == "group") {
                    def coassemblygroup = row.group
                    //.ifEmpty(exit 1, "Invalid input samplesheet: No group column for coassembly was found")
                    if (coassemblygroup == "null" || coassemblygroup == "") {
                        exit(1, "Invalid input samplesheet: No group column for coassembly was found or contains empty fields")
                    }
                    ...
                    def binninggroup = row.group
                    // reuse group annotation for co-binning with vamb instead of coassembly when available
                    if (binninggroup && binninggroup != "null") {
                        meta.binninggroup = binninggroup
                    }
                    else {
                        meta.binninggroup = meta.id
                    }
                }
                else {
        ...
        download_files(rawoutput.download.map { it ->
            return [it[0], it[2]]
        })
        ch_mix = channel.empty().mix(rawoutput.local.map { it ->
            return [it[0], it[2]]
        }).mix(download_files.out.reads)
        inreads = ch_mix
    }
    else {
        channel.fromFilePairs(params.reads, size: params.single_end ? 1 : (params.no_qc ? (params.only_paired ? 2 : 3) : 2))
            .ifEmpty {
                exit(
                    1,
                    (!params.no_qc
                        ? "Cannot find any matching reads in ${params.reads}\nPaths must be in enclosed quotes.\nIf you have single-end reads, make sure to add the parameter --single_end."
...
workflow input_sra {
    ...
    main:
    def ids = params.sra.toString().split(',').collect { accession -> accession.replaceAll("\\s", "").toUpperCase() }.findAll { accession -> accession }
    if (params.ena_query) {
        ...
        ena_query.out
            ...
            .map { row ->
                ...
                def readsize = reads.count { read -> read }
                meta.single_end = readsize == 1 ? true : false
                if (params.assemblymode == "single") {
                    meta.coassemblygroup = meta.id
                    meta.binninggroup = meta.id
        ...
        if (!params.exact_matches) {
            ...
            rawoutput = raw_rawoutput.filter { meta, _files ->
        ...
        if (!params.apikey) {
            ...
            channel.fromSRA(ids, apiKey: params.apikey)
                .map { row ->
                    ...
                    def fastq = row[1] instanceof List ? row[1] : [row[1]]
                    def readsize = fastq.size()
                    meta.single_end = readsize == 1
                    if (readsize == 3) {
                        ...
                        if (hasExtension(fastq[0], ".fastq.gz")) {
                            //order shall be: meta, unpaired
                            return [meta, fastq[0]]
                        }
                        else {
                            println("Invalid file paths in single-end ${fastq[0]}, it does not end with .fastq.gz")
                        }
                    }
                }
                .set { raw_rawoutput }
        ...
        if (!params.step1) {
            ...
            ch_collectedinput
                ...
                .map { row ->
                    ...
                    if (!read1) {
                        exit(1, "Invalid input samplesheet in at least four columns! Is your tsv file tab separated?")
                    }
                    def read2 = row[4] ? row[4].replaceAll(/\]/, "").replaceAll(/ /, "") : null
                    def read3 = row[5] ? row[5].replaceAll(/\]/, "").replaceAll(/ /, "") : null
                    if (!hasExtension(read1, ".fastq.gz")) {
                        exit(1, "Invalid read names! Reads need to end with .fastq.gz")
                    }
                    def meta = [:]
                    meta.id = row[0]
                    def singleEnd = row[1]
                    ...
                    meta.binninggroup = meta.coassemblygroup
                    def fastq = [read1, read2, read3].findAll { read -> read && read != "null" }
                    if (fastq.size() == 3) {
                        if (hasExtension(fastq[1], ".fastq.gz") && hasExtension(fastq[2], ".fastq.gz") && hasExtension(fastq[0], ".fastq.gz")) {
                            //order shall be: meta, forward, reversed, unpaired
                            return [meta, [fastq[1], fastq[2], fastq[0]]]
